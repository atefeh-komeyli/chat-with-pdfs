...
def test_get_ollama_llm_connection_error(mock_ollama):
    ...
    with pytest.raises(ConnectionError) as excinfo:
        main.get_ollama_llm()
    assert f"Failed to connect to Ollama model {main.OLLAMA_MODEL}" in str(
        excinfo.value
    )
...
def test_process_pdfs_success(
    mock_pdf_loader, mock_splitter, mock_chroma, mock_create_chain, mock_get_llm
):
...
def test_handle_upload_success(mock_process_pdfs, mock_get_embedding_function):
    ...
    mock_process_pdfs.assert_called_once_with(
        mock_files, mock_get_embedding_function.return_value
    )
...
def test_handle_chat_chain_not_initialized():
    ...
    try:
        ...
        assert (
            "Error: The RAG chain is not initialized" in updated_history[1]["content"]
        )
        assert msg == ""
    finally:
